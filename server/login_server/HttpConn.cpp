...
//该方法会先尝试着调用底层的send()函数去发送，如果不能全部发送出去，
//则将剩余数据加入到对应的写数据缓冲区内。这样这些数据会在该socket
//可写时再继续发送。这是也是设计网络通信库一个通用的技巧，即先试着
//去send，如果send不了，将数据放入待发送缓冲区内，并设置检测可写标
//识位，当socket可写时，从待发送缓冲区取出数据发送出去。如果还是不
//能全部发送出去，继续设置检测可写标识位，下次再次发送，如此循环一
//直到所有数据都发送出去为止
int CHttpConn::Send(void* data, int len)
{
    ...
    {
        OnWriteComlete(); //http设置成了短连接，每次应答完客户端应断开连接
    }
...
void CHttpConn::OnConnect(net_handle_t handle)
{
	...
	//设置http数据的回调函数httpconn_callback

...
void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	...
	ConnMap_t* conn_map = (ConnMap_t*)callback_data; //这个callback_data也是在连接阶段连同回调函数被一起设置的
	CImConn* pConn = FindImConn(conn_map, handle);   //pConn指向的是CImConn的子类对象,该类继承自CImConn
	if (!pConn)
		return;

	//log("msg=%d, handle=%d ", msg, handle);

	switch (msg)	 //此处根据具体的Pconn指向的子类对象，多态实现调用具体子类的响应函数，比如CproxyConn对象等
	{

...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	_SetAddr(server_ip, port, &serv_addr); //注意gethostbyname的使用,参考TCP/IP 131页
	...
	//将this指向的socket也就是本socket加入hash_map中,AddEvent设置需要关注的socket事件,此处仅关注可读和出错事件
	AddBaseSocket(this);
...
net_handle_t CBaseSocket::Connect(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	AddBaseSocket(this);
	CEventDispatch::Instance()->AddEvent(m_socket, SOCKET_ALL); //关注该socket的所有事件
...
int CBaseSocket::Send(void* buf, int len)
{
	...
	if (ret == SOCKET_ERROR)
	{
		...
		{
#if ((defined _WIN32) || (defined __APPLE__))  //疑问：这里Linux为什么不添加写事件呢？猜测Linux的epoll如果没有写完会再次注册写事件？？？
...
void CBaseSocket::OnRead()
{
	...
	{
		//如果是侦听的socket需要读,则表明有连接请求,新建socket并保存这个连接字
		_AcceptNewSocket();
	}
	else
	{
		u_long avail = 0;
		//调用ioctlsocket函数获得可读的数据字节数,如果出错或者字节为0,则callback CLOSE
...
{
#if ((defined _WIN32) || (defined __APPLE__)) //同样这里为什么没有取消Linux下的写事件？猜测是如果写完后自动注销？？？
	CEventDispatch::Instance()->RemoveEvent(m_socket, SOCKET_WRITE);
#endif
	//区分是尝试connect连接的socket还是已经连接的socket
	if (m_state == SOCKET_STATE_CONNECTING)
	{
		...
		if (error) {
			...
			m_state = SOCKET_STATE_CONNECTED; //将socket的状态设置为CONNECTED的,并调用该socket创建时绑定的回调函数
			m_callback(m_callback_data, NETLIB_MSG_CONFIRM, (net_handle_t)m_socket, NULL);
		}
	}
	//否则是已经与客户端保持连接的socket
	else
	{
...
//将标识m_state设置为需要关闭状态，并调用预先设置的回调函数
...
{
	int reuse = 1;
	//SO_REUSEADDR允许套接字和一个已经在使用中的地址捆绑
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		//调用初始化阶段netlib_listen设置的回调函数http_callback
